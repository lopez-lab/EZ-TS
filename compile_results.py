...
def get_cis(barriers,input):
    ...
    for mol,row in forward.iterrows():
        for col in cols:
            filename = '{1}/reactants/{0}-forward.log'.format(mol,input)
    ...
    for mol,row in reverse.iterrows():
        for col in cols:
            filename = '{1}/reactants/{0}-reverse.log'.format(mol,input)
            if os.path.exists(filename):
                if verify_finished(filename,'reactant'):
                    for line in open(filename,'r'):
                        if re.search('Sum of electronic and thermal Free Energies',line):
    ...
    for mol,row in higher_energy.iterrows():
        ...
        #ME
                ...
                cis = forward[col][mol]
                shutil.copy('{1}/reactants/{0}-reverse.log'.format(mol,input),'{0}/trans/{1}-reverse.log'.format(input,mol))
                shutil.copy('{1}/reactants/{0}-forward.log'.format(mol,input),'{0}/cis/{1}-forward.log'.format(input,mol))
            elif difference < 0. :
                cis = reverse[col][mol]
                shutil.copy('{1}/reactants/{0}-forward.log'.format(mol,input),'{0}/trans/{1}-forward.log'.format(input,mol))
                shutil.copy('{1}/reactants/{0}-reverse.log'.format(mol,input),'{0}/cis/{1}-reverse.log'.format(input,mol))
            else:
                cis = 'NaN'
...
def get_ts(barriers,input):
    ...
    for mol,row in ts.iterrows():
        ...
        for col in cols:
            filename = '{1}/lowest_ts/{0}.log'.format(mol,input)
            if os.path.exists(filename):
...
def main():
    ...
    for (input,input_name) in zip(inputs,inputs_names):
        ...
        if os.path.isdir('{0}/benchmarking'.format(input)):
            ...
            print('''Found benchmarking dir at: {0}
...
#        barrier_columns = [com.split('{0}-'.format(mols[0]))[-1].split('.')[0] for com in files[0]]
        barrier_columns = ['Barrier (kcal/mol)']
